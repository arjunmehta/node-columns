...
function ColumnView(column) {
    ...
    heartbeats.heart('view_refresh').createEvent(1, function(heartbeat, last) {
        ...
        threshold = (buffer.new_lines < _this.data_height || buffer.new_lines > 5000 || buffer.new_lines === new_lines) ? true : false;
        new_lines = buffer.new_lines;

        if (threshold === true) {
            if (printing === false) {
                printing = true;
                _this.renderBuffer(buffer.new_lines, function() {
                    printing = false;
                });
                _this.buffer.reset();
            }
...
ColumnView.prototype.renderBuffer = function(new_lines, done) {

    if (this.column.displaying === true) {

        if (this.column.columns.opts.flow === true) {
            if (new_lines > 0 || this.buffer.current_modified === true) {
                this.redrawDataFlow();
                this.buffer.reset();
                done();
            }
            ...
            if (new_lines > 0) {
                this.printNewLines(new_lines, done);
            } else {
                done();
            }
        }
...
ColumnView.prototype.printNewLines = function(num_lines, done) {

    var printed = 0;

    if (num_lines > this.data_height) {
        this.current_line += num_lines - this.data_height;
        num_lines = this.data_height;
    }

    for (var i = 0; i < num_lines; i++) {
        this.current_line++;
        this.printBufferLine(this.current_line, cb);
    }

    function cb() {
        printed++;
        if (printed === num_lines) {
            process.stdout.cursorTo(100, 0);
            process.stdout.write("!!!!!!!!!!!!DONE " + num_lines);
            done();
        }
    }
};

ColumnView.prototype.printBufferLine = function(buffer_index, cb) {

    // process.stdout.cursorTo(0, 0);
    // process.stdout.write("current_line" + ' ' + this.current_line + ' ' +  buffer_index + " something " + this.buffer.size);
    ...
    for (var i = 0; i < line_array.length; i++) {
        this.printLine(line_array[i], _cb);
    }

    function _cb() {
        printed++;
        if (printed === line_array.length && typeof cb === 'function') {
            cb();
        }
    }
};
...
ColumnView.prototype.printLine = function(line, cb) {
    ...
    this.printLineDirect(line, cb);
};

ColumnView.prototype.printLineDirect = function(line, cb) {

    this.dataCursorPos(this.cursorY);
    writeToScreen(line, cb);
    this.cursorY++;
};
...
function writeToScreen(line, cb) {
    process.stdout.write(line, cb);
}
