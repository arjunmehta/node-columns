...
ColumnView.prototype.redrawAll = function(separator) {
    ...
    if (this.main.opts.flow === true && this.stream.data.length > 0) {
        this.redrawDataFlow();
    } else {
        this.printToCurrent();
...
function trimLine(line, col_width) {

    var i = 0,
        line_codes = codes.strip(line),
        truncated = line.substring(0, col_width + i),
        width = wcwidth(truncated);

    line = line_codes.line;
    ...
    return codes.reinsert(truncated, line_codes);
}

function codeQueue(new_codes){
    if(new_codes.indexOf(0) > -1){
