...
ColumnView.prototype.redrawAll = function(separator) {
    ...
    if (this.main.flow === true && this.stream.data.length > 0) {
        this.redrawDataFlow();
    } else {
        this.printToCurrent();
...
ColumnView.prototype.redrawDataEffeciently = function() {
    ...
    var data_length = this.stream.data.length;
    var modulo = this.current_line > this.data_height + this.overflow ? (this.current_line - this.overflow) % (this.data_height - this.overflow) : this.current_line % this.data_height;
    ...
    this.modulo = modulo;
    this.cursorY = overflow ? modulo + this.overflow : modulo;

    this.printLine(this.stream.data[this.current_line]);
...
ColumnView.prototype.printToCurrent = function() {
    ...
    var overflow = (this.current_line < this.data_height) ? false : true;
    var numLines = overflow ? this.modulo + this.overflow : this.modulo;

    this.cursorY = 0;

    for (var i = this.current_line - numLines; i < this.current_line; i++) {
        this.printLine(this.stream.data[i]);
    }
};
...
ColumnView.prototype.printOverflow = function() {
    ...
    this.cursorY = 0;
    for (var i = this.current_line - this.overflow; i < this.current_line; i++) {
        this.printLine(this.stream.data[i]);
    }
};
...
ColumnView.prototype.redrawDataFlow = function() {
    ...
    var offset = (data.length > this.data_height) ? this.data_height : data.length;
