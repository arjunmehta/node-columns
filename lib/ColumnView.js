...
ColumnView.prototype.printToOverflow = function() {

    var overflow_line = this.current_line;
    var line_array = this.buffer.line(overflow_line).trimmed(this.width, this.wrap);

    var i = this.overflow - 1,
        j = line_array.length - 1;

    while (i >= 0) {
        this.cursorY = i;
        this.printLine(line[j], false);
        j--;
        if (j < 0) {
            overflow_line--;
            line_array = this.buffer.line(overflow_line).trimmed(this.width, this.wrap);
            j = line_array.length - 1;
        }
        i--;
    }
