...
ColumnLine.prototype.trimmed = function(to_length, wrap, offset, lineArray) {

    lineArray = lineArray === undefined ? [] : lineArray;
    offset = offset === undefined ? 0 : offset;

    var i = 0,
        line = (this.replaceTabs(this.codes.strippedLine) + MainView.empty_line).substring(offset, to_length),
        truncated = line.substring(offset, to_length),
        truncated_width = width(truncated);

    while (truncated_width > to_length) {
        i++;
        truncated = line.substring(offset, to_length - i);
        truncated_width = width(truncated);
    }

    lineArray.push(this.insertCodes(truncated, offset));

    //handle wrapping
    if (wrap === true && this.codes.strippedLine.length > to_length + offset) {
        offset = to_length - i;
        lineArray = lineArray.concat(this.trimmed(to_length, wrap, offset, lineArray));
    }
